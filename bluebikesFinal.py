...
def main():
    ...
    # !!!!!!!! STREAM LIT PIZZA EXCERISE !!!!!!
    st.divider()
    st.header("Rising Dough Pizzeria Menu")
    
    MENU_PRICES = {
        'Margherita': {'Small': 9.00, 'Medium': 12.00, 'Large': 15.00},
        'Pepperoni': {'Small': 10.00, 'Medium': 13.50, 'Large': 17.00},
        'Veggie': {'Small': 9.50, 'Medium': 12.50, 'Large': 15.50},
        'Hawaiian': {'Small': 11.00, 'Medium': 14.00, 'Large': 17.50}
    }
    ...
    PIZZA_DESCRIPTIONS = {
        'Margherita': 'Classic tomato sauce, fresh mozzarella, basil, and olive oil.',
        'Pepperoni': 'Tomato sauce, mozzarella, and spicy pepperoni slices.',
        'Veggie': 'Tomato sauce, mozzarella, bell peppers, onions, mushrooms, and olives.',
        'Hawaiian': 'Tomato sauce, mozzarella, ham, and pineapple chunks.'
    }

    col1, col2 = st.columns(2)

    with col1:
        selected_pizza = st.selectbox("Choose your base pizza:", list(MENU_PRICES.keys()))
        selected_size  = st.radio("Choose your size:", ['Small (10")', 'Medium (12")', 'Large (14")'])
        selected_crust = st.radio("Choose your crust:", list(CRUST_COSTS.keys()))

    with col2:
        selected_toppings = st.multiselect(
            "Add Toppings (Up to 6):",
            list(TOPPING_COSTS.keys()),
            max_selections=6
        )

    st.info(f"**{selected_pizza}:** {PIZZA_DESCRIPTIONS[selected_pizza]}") 
    
    size_key = selected_size.split(" ")[0]

    base_price = MENU_PRICES[selected_pizza][size_key]
    crust_price = CRUST_COSTS[selected_crust]
    topping_price = sum(TOPPING_COSTS[t] for t in selected_toppings)
    total_price = base_price + crust_price + topping_price
    
    st.subheader("Pizza Order Summuary")
    st.write(f"**Base ({selected_pizza}, {size_key}):** ${base_price:.2f}")
    st.write(f"**Crust ({selected_crust}):** +${crust_price:.2f}")

    if selected_toppings:
        for topping in selected_toppings:
            st.write(f"**{topping}:** +${TOPPING_COSTS[topping]:.2f}")

    st.metric("Total Price", f"${total_price:.2f}")

    st.divider()
    st.header("Project Feature Checklist")

    feature_dict = {
        "Python features": "functions, multiple return values, list comprehension, lambda",
        "Streamlit features": "dropdown, slider, sidebar, page layout",
        "Visualizations": "bar chart, horizontal bar chart, PyDeck map",
        "Data analytics": "filters, sorting, pivot table, new columns, max value"
    }

    for feature, description in feature_dict.items():
        st.write(f"**{feature}:** {description}")

    st.write("Feature categories used:", list(feature_dict.keys()))
    st.write("Feature descriptions:", list(feature_dict.values()))
